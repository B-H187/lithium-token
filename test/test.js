...
contract('dBank', ([deployer, user]) => {
  ...
  beforeEach(async () => {
    lithium = await Lithium.new()
    dbank = await DecentralizedBank.new(lithium.address)
    await lithium.passMinterRole(dbank.address, {from: deployer})
  })

  describe('testing token contract...', () => {
    describe('success', () => {
      it('checking token name', async () => {
        expect(await lithium.name()).to.be.eq('Lithium')
      })

      it('checking token symbol', async () => {
        expect(await lithium.symbol()).to.be.eq('LTH')
      })

      it('checking token initial total supply', async () => {
        expect(Number(await lithium.totalSupply())).to.eq(0)
      })

      it('dBank should have Token minter role', async () => {
        expect(await lithium.minter()).to.eq(dbank.address)
      })
    })

    describe('failure', () => {
      it('passing minter role should be rejected', async () => {
        await lithium.passMinterRole(user, {from: deployer}).should.be.rejectedWith(EVM_REVERT)
      })

      it('tokens minting should be rejected', async () => {
        await lithium.mint(user, '1', {from: deployer}).should.be.rejectedWith(EVM_REVERT) //unauthorized minter
      })
    })
  })
  ...
  describe('testing withdraw...', () => {
    ...
    describe('success', () => {
      ...
      it('user should receive proper amount of interest', async () => {
        ...
        balance = Number(await lithium.balanceOf(user))
